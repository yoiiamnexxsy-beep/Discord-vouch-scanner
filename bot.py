...
# Custom server emojis used in embeds
OWO_EMOJI = "<:owo:1493524875757748384>"
LTC_EMOJI = "<:LTC:1493524815779467274>"
...
async def scanvouches(ctx, channel: discord.TextChannel = None, limit: int = None):
    ...
    embed.add_field(name=f"{OWO_EMOJI} Total OWO", value=f"{total_owo:,.0f}", inline=False)

    ltc_line = f"${usd_value:,.2f}" if usd_value is not None else f"{total_ltc:,.4f} LTC (rate unavailable)"
    embed.add_field(name=f"{LTC_EMOJI} Total LTC ($)", value=ltc_line, inline=False)
...
async def vouchstats(ctx, member: discord.Member = None):
    ...
    usd_rate, _ = await get_ltc_rates()
    usd_value = stats["ltc"] * usd_rate if usd_rate else None
    ...
    embed.add_field(name=f"{OWO_EMOJI} OWO", value=f"{stats['owo']:,.0f}", inline=True)
    ltc_display = f"${usd_value:,.2f}" if usd_value is not None else f"{stats['ltc']:.6f} LTC"
    embed.add_field(name=f"{LTC_EMOJI} LTC ($)", value=ltc_display, inline=True)
...
async def leaderboard(ctx):
    ...
    for i, (user_id, owo_total, ltc_total, inr_total, count) in enumerate(rows, start=1):
        ltc_display = f"${ltc_total * usd_rate:,.2f}" if usd_rate else f"{ltc_total:.4f} LTC"
        lines.append(
            f"**{i}. <@{user_id}>** — {owo_total:,.0f} {OWO_EMOJI} | {ltc_display} {LTC_EMOJI} | "
...
async def serverstats(ctx):
    ...
    usd_rate, _ = await get_ltc_rates()
    usd_value = totals["ltc"] * usd_rate if usd_rate else None
    ...
    embed.add_field(name=f"{OWO_EMOJI} Total OWO Given", value=f"{totals['owo']:,.0f}", inline=True)
    ltc_display = f"${usd_value:,.2f}" if usd_value is not None else f"{totals['ltc']:.6f} LTC"
    embed.add_field(name=f"{LTC_EMOJI} Total LTC Given ($)", value=ltc_display, inline=True)
